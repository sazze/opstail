...
Ops.prototype = {
   ...
   */
  _connect: function() {
    log('Connecting...', log.GREEN);

    this._socket = client(`ws://${this.options.host}:${this.options.port}`, {
  ...
  _handleMessage: function(message) {
    ...
    if (!this._isTypeEnabled(opsMessage.errorType) && !opsMessage.isSand) {
      return;
    }
  ...
  _printMessage: function(opsMessage) {
    if ((!this._isTypeEnabled(opsMessage.errorType) && !opsMessage.isSand) || !this.printMessages) {
      return;
    }
    ...
    _.each(this.messages, function(msg) {
      ...
      try {
        if (this.options.numTraces && count < (this.messages.length - this.options.numTraces)) {
          this.options.tracesEnabled[msg.errorType] = false;
        } else {
          this.options.tracesEnabled[msg.errorType] = original[msg.errorType];
        }
      } catch (e) {}
